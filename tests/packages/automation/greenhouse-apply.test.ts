...
describe('greenhouse apply site flow', () => {
  ...
  test('autofills sponsorship from minimal profile when requiresSponsorship is set', async () => {
    server = createServer((request, response) => {
      const url = request.url ?? '/';
    ...
    await runGreenhouseApply({
      ...
      applicant: {
        ...
        country: 'Canada',
        resumePath,
        autofillProfile: minimalAutofillProfileSchema.parse({ requiresSponsorship: 'no' }),
        profileContext: { summary: '', reusableContext: '' }
      },
      helpers: {
        mapField: async (step) => {
          await page.locator(step.selector).fill(step.value);
  ...
  test('pauses for sponsorship when minimal autofill profile does not set sponsorship', async () => {
    server = createServer((request, response) => {
      const url = request.url ?? '/';

...
export async function completeApplicationRun(input: {
  ...
  const filePrefix = `${sanitizeSegment(input.siteKey)}-${sanitizeSegment(
    input.step
  )}`;
  ...
  const evidenceDetails = {
    ...
    step: input.step,
    pageUrl: input.reviewUrl ?? input.page.url(),
    ...(input.details ?? {})
  };
  ...
  return input.applicationRunsRepository.update(input.run.id, {
    ...
    stopReason: null,
    reviewUrl: input.reviewUrl ?? input.page.url(),

...
const RotatingText = forwardRef<RotatingTextHandle, RotatingTextProps>((props, ref) => {
  ...
  const next = useCallback(() => {
    ...
    if (randomize) {
      ...
      if (nextIndex === currentTextIndex && texts.length > 1) {
        nextIndex = (nextIndex + 1) % texts.length;
      }
    } else {
      nextIndex = currentTextIndex === texts.length - 1 ? (loop ? 0 : currentTextIndex) : currentTextIndex + 1;
    }
    if (nextIndex !== currentTextIndex) {
      handleIndexChange(nextIndex);
    }
  }, [currentTextIndex, texts.length, loop, handleIndexChange, randomize]);
